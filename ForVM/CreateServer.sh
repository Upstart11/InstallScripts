...
echo "if ! sudo -u $USER_NAME \$PYTHON_PATH \$COMFYUI_RUNNER_PATH --listen --port 8888; then" >> run_the_server.sh
...
echo " "
echo -e " ---------------- For health check go to this address $(curl -s httpbin.org/ip | jq -r .origin):8888 in your browser"